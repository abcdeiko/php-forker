...
class Job_Manager {
	...
	// check running jobs
	// if it's find a stopped job, we will unset pipes
		$running_jobs = 0;
	...
	public function startJob($cmd, $name = 'job') {
		...
		// зачем проверяем работу на старте????
		// может быть если вдруг кто второй раз собирается запустить 
		$this->checkJobs();
	...
	// delete pipes pointer for one job
